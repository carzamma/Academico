...
Al conocer las condiciones que se deben cumplir, es posible utilizar el software estadístico R para generar una función[^5] brinde los datos simulados adecuados. Como los coeficientes no son de particular interés en este estudio se plantea un escenario básico pero que puede ser fácilmente modificado: se definirá un único predictor llamado $x$ con el cual se creará un modelo lineal generalizado en conjunto con una variable de respuesta $y$, que serán los números pseudoaleatorios de la distribución de Poisson con sobredispersión.              
[^5]: Ver anexos para más información del código empleado.
...
Como método de comparación se estimarán los errores estándar de un modelo lineal generalizado mediante bootstrap[^6], tanto para el modelo Poisso como el Quasipoisson; estos errores serán comparados contra los generados por los modelos lineales generalizados clásicos de Poisson y de Quasipoisson.       

[^6]: Buscar referencia de boostrap
...
geom_point(col="blue")+
facet_grid(.~n)+
labs(title="Gráfico 2\nDiferencia global del error estándar con respecto al factor de dispersión para cada tamaño de muestra", 
     x=expression(paste(phi)),
     y="Diferencia")
```
